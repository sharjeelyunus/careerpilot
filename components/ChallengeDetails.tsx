...
import { TechnicalChallenge, ChallengeSubmission } from '@/lib/store/technicalChallengesStore';
import { submitChallengeSolution } from '@/lib/actions/technicalChallenges.action';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Clock, Star, Trophy, AlertCircle, CheckCircle2, Code2, History, Terminal } from 'lucide-react';
...
export function ChallengeDetails({ challenge }: ChallengeDetailsProps) {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState(() => {
    // Try to find a matching language from the tech stack
  ...
  const bestSubmission = sortedSubmissions?.reduce<ChallengeSubmission | null>((best, current) => 
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitChallengeSolution(challenge.id, solution, user.id);
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-light-100">{challenge.title}</h1>
              <div className="flex items-center gap-3 text-light-100/70">
                ...
                {bestSubmission && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm">Best: {bestSubmission.feedback.score}/100</span>
                  </div>
                )}
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Badge variant="outline" className="bg-primary-200/10 text-primary-200 border-primary-200/20">
                {challenge.difficulty}
              </Badge>
              {challenge.techStack.map((tech) => (
                <Badge key={tech} variant="secondary" className="bg-dark-300/50">
                  {tech}
                </Badge>
              ))}
            </div>
          ...
          <Card className="bg-dark-200/30 border-primary-200/10">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-semibold text-light-100">Your Solution</CardTitle>
            ...
            <CardFooter className="flex flex-col gap-4">
              ...
              <Button 
                ...
                className="w-full bg-primary-200 hover:bg-primary-200/90 text-dark-100 font-medium"
              >
                {isSubmitting ? (
                  <>
          ...
          {feedback && (
            ...
            >
              <div className="flex items-center justify-between border-b border-primary-200/10 pb-4">
                <h3 className="text-xl font-semibold text-light-100">Feedback</h3>
        ...
        <TabsContent value="submissions" className="mt-4">
          {sortedSubmissions && sortedSubmissions.length > 0 ? (
              ...
              <Accordion type="single" collapsible className="space-y-4">
                {sortedSubmissions.map((submission, index) => (
                  ...
                  >
                    ...
                    >
                      <AccordionTrigger 
                        className="px-6 py-4 hover:no-underline group w-full"
                      >
            ...
            <div className="flex flex-col items-center justify-center bg-dark-200/30 rounded-xl border border-primary-200/10 p-8 text-center">
              ...
              <h3 className="text-lg font-medium text-light-100 mb-2">No submissions yet</h3>
              <p className="text-light-100/70">Submit your first solution to see your attempts here.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
